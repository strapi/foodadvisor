    ...
    <footer className="bg-white mt-24 pt-4 pb-8 xl:pt-8">
      <div className="max-w-screen-lg xl:max-w-screen-xl mx-auto px-4 sm:px-6 md:px-8 text-gray-400 dark:text-gray-300">
                  ...
                  <ul>
                    {column.links &&
                      column.links.map((link, index2) => (
                        <li className="mb-4 hover:text-gray-800 dark:hover:text-white transition-colors duration-200" key={`footerColumnLink-${index2}`}>
                          <Link href={link.href}>
        ...
        <div className="text-center pt-10 sm:pt-12 font-light flex items-center justify-center">
          {footer.button && (
            <button
              className={`flex-shrink-0 px-4 py-2 text-base font-semibold text-white bg-${footer.button.theme} rounded-lg shadow-md hover:bg-${footer.button.theme}-darker focus:outline-none focus:ring-2`}
              type="submit"
            >
              <Link href={footer.button.link.href}>
                <a target={footer.button.link.target}>
                  {footer.button.link.label}
                </a>
              </Link>
            </button>
          )}
