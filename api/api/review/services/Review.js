...
module.exports = {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Review.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Review._attributes)
      .filter(attribute => attribute !== Review.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Review._attributes[attribute].type));
