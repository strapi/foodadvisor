...
export class Admin extends React.Component {
  ...
  renderInjectedOnboardingComponents = () => {
    const {
      strapi: {
        strapi: { getPlugin },
      },
    ...
    const injectedComponents = getPlugin('admin').getInjectedComponents('admin', 'onboarding');

    const compos = injectedComponents.map((compo) => {
      return <compo.Component key={compo.name} />;
    });

    return [...compos, <OnboardingVideos key="onboarding-videos" />];
  };
        ...
        <UserProvider value={userPermissions}>
          <Wrapper>
            ...
            <OverlayBlocker
              ...
              {...overlayBlockerData}
            />
            {SHOW_TUTORIALS && this.renderInjectedOnboardingComponents()}
          </Wrapper>
        </UserProvider>
      </GlobalContextProvider>
    );
